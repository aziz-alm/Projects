...
I searched for the old name online and found a legacy company website still live on a third-party hosting platform. The site had a team page listing employee names and email addresses, all under a completely different domain. Both domains pointed to the same MX records, so I checked whether they shared the same M365 tenant by querying Microsoft's public OpenID configuration endpoint. The tenant IDs matched — same tenant, two domains.
...
![PoC — spoofed email in inbox](yahoo_screenshot__1_.png)

![PoC — sender address detail](preview__1_.webp)
